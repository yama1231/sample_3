<?php
    ...
    $ban = isset($_POST['ban']) ? (int)$_POST['ban'] : null;
    ...
    //「予約フォーム」からの情報をそれぞれ変数に格納しておく↑
    // ホスト名：db　データベース名:myapp
    try{
        $stmt = $pdo->prepare("INSERT INTO sample (ban, name, number, member, day) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$ban, $name, $number, $member, $day]);
    }catch (Exception $e) {
    ...
    try {
        ...
        $error_message = "データ取得に失敗しました: " . $e->getMessage();
        $reservation_data = [];
    }
